...
describe('Booster NFT', function () {
  ...
  before(async function () {
    ...
    mysteryBoxNFTContract = await deployContract('MysteryBoxNFT', [ownerWallet.address], {
      wallet: ownerWallet,
      silent: true,
      noVerify: true,
    });
    ...
    boosterNFTContract = await deployContract('BoosterNFT', [ownerWallet.address, mysteryBoxNFTContractAddress], {
      wallet: ownerWallet,
      silent: true,
      noVerify: true,
    });
  ...
  it('Should burn the Box and mint new Booster', async function () {
    ...
    const tx = await mysteryBoxNFTContract['safeMint(address, bytes, string, uint256)'](
      address,
      signature,
      String(nonce),
      expiry,
    ...
    // Mint Booster
    const tx3 = await boosterNFTContract['safeMint(address, uint256, string, string, uint256, bytes)'](
      address,
      BigInt('0'),
      type_of_booster,
      String(nonce),
