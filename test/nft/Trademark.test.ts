...
describe('Trademark NFT', function () {
  ...
  before(async function () {
    ...
    nftContract = await deployContract('TrademarkNFT', [ownerWallet.address], {
      wallet: ownerWallet,
      silent: true,
      noVerify: true,
    });
  ...
  it('Should mint a new NFT to the recipient', async function () {
    ...
    const signature = getSignature(address, `NOVA-TradeMark-${type_of_nft}-${nonce}`, ownerWallet.privateKey || '');
