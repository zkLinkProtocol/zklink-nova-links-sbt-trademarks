...
export default async function () {
  ...
  const approve_all_transaction = await mysteryBoxNFTContract['setApprovalForAll(address, bool)'](
    BOOSTER_CONTRACT_ADDRESS,
    true,
  );
  ...
  const safe_mint_transaction = await boosterContract['safeMint(uint256, string, uint256, uint256, bytes)'](
    ...
    nonce,
    expiry,
    getSignature(ACCOUNT_ADDRESS, 'NOVA-BOOSTER-SBT-500', process.env.WITNESS_SINGER_PRIVATE_KEY || ''),
  );
