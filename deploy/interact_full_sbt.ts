...
export default async function () {
  ...
  const approve_all_transaction = await tradeMarkNFTContract['setApprovalForAll(address, bool)'](
    FULL_NOVA_NFT_CONTRACT_ADDRESS,
    true,
  );
  ...
  const safe_mint_transaction = await fullNovaContract[
    'safeMint(uint256, string, uint256, uint256, uint256, uint256, bytes, string, uint256)'
  ](
    tokenId,
    'ESFJ',
    1,
    ...
    3,
    4,
    getSignature(ACCOUNT_ADDRESS, 'NOVA-LYNK-1', process.env.WITNESS_SINGER_PRIVATE_KEY || ''),
    String(nonce),
    expiry,
  );
